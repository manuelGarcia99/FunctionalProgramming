...
let rec valorDaLista = 
  ...
  let rec desceTabela  (tabela : (int , int) Hashtbl.t) (posicaoAtual : int) (menorKey : int)=(**usar list rev e usar :: e nunca o @ n*(n+1)/2 > *)
...
let rec main fatias tabelaDosPrecos numDePartida numAEntrar numMenor
 ...
 (*let () = print_string("SHI") in*) 
  ...
  if (numDePartida = numMenor && valorDoArray arrNumDeFatiasDeTamanho > fatias - numMenor) then(
    ...
    valorDoArray arrNumDeFatiasDeTamanho <= fatias ) then( 
     if(not(contemPreco tabelaDosPrecos arrNumDeFatiasDeTamanho listaDeLucros) && precoDoArray tabelaDosPrecos arrNumDeFatiasDeTamanho >= minimo ) 
    ...
    else(*3*)if  (valorDoArray arrNumDeFatiasDeTamanho >= fatias - numMenor) then( if( numAEntrar = numMenor)(*Para tirar*) then( if arrayContemFatiasDeUmSoTamanho arrNumDeFatiasDeTamanho && ((arrNumDeFatiasDeTamanho.(numDePartida)) <> 0) then (
      let() = print_int(numDePartida) in
      ...
      numDePartida numDePartida numDePartida numMenor minimo listaDeLucros) )(*Combinações*)
    )
